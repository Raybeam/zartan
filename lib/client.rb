class Client
  ...
  def reserve_proxy(site, proxy_id, proxy_ts)
    api_log(event: 'reserve', client_id: self.id, site: site, proxy_id: proxy_id, ready_unix_time: proxy_ts)
  ...
  def get_proxy(site, older_than)
    api_log(event: 'request', client_id: self.id, site: site, older_than: older_than)
    ...
    if proxy_ts.nil?
      ...
      if result.is_a? Proxy::NotReady
        ...
        api_log(event: 'found_proxy', client_id: self.id, site: site, proxy_id: result.id)
      end
    else
      ...
      if proxy_ts > threshold_ts
        ...
        api_log(event: 'reservation_not_ready', client_id: self.id, site: site)
        result = Proxy::NotReady.new(proxy_ts, threshold_ts, proxy_id)
      else
        # We have a chached proxy ready for use.
        api_log(event: 'reservation_ready', client_id: self.id, site: site, proxy_id: proxy_id)
